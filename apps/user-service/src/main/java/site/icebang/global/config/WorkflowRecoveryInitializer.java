 ...
 *
 * <p>서버가 시작될 때 DB에 'RUNNING' 상태로 남아있는 워크플로우는
 * 시스템 장애(전원 차단, OOM 등)로 인해 중단된 작업입니다.
 * 이 클래스는 해당 작업들을 찾아내어 이전 기록을 정리하고, 작업을 <b>자동으로 재시작</b>합니다.
 */
@Slf4j
@Component
...
public class WorkflowRecoveryInitializer implements ApplicationRunner {
    ...
    private boolean recoverAndRestart(WorkflowRun run) {
        try {
            ...
            log.info("[복구 2단계] 워크플로우 재실행 요청 완료: WorkflowID={}, TraceID={}", run.getWorkflowId(), run.getTraceId());
